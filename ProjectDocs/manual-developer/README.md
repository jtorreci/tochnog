...
- [group_element_selective_reduced_integration](group_element_selective_reduced_integration.md) — the CLASSIC shear locking / parasitic shear of the bilinear quad4 in bending (u_y ~ -κ·x²/2 not representable -> spurious γ_xy -> too stiff; measured: 0.231× moment, 0.741× shear) and the opt-in fix: selective reduced integration (shear at 1 Gauss point at the centroid, normal terms at the full 2×2 Gauss rule). Scope: elastic 2D quad4 only; the codebase's default Lobatto corner rule would cap the benefit (documented); solver GOTCHA of the mixed u-σ scheme documented (classic 1.0× reference = 99.3% deflection / 93.75% moment). Family `qsri` (8 tests).
