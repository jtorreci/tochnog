...
- **Implementado**: `group_materi_plasti_mohr_coul_direct` (+ `_normal`,
  `_normal_automatic`) and `group_materi_plasti_tension_direct` (+ `_normal`,
  `_normal_automatic`). Validated with `materi_direct` (uniaxial tension
  capped to sigy), `materi_direct_mc` (pure shear capped to max_fric) and
  `materi_direct_auto` (element-normal automatic).
- **Pendiente**: `_visco` variants, softening via dependency diagrams.
