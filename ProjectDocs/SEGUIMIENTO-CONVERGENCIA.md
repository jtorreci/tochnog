...
- `patch1`: **NO es un bloqueador de solver** (diagnóstico corregido 2026-09-04, sprint solver c2cbaf7). Medido: el σn de la interfaz es 1431.077 IDÉNTICO con el Bi-CG (1431.05), con la banda LU directa (`solver -matrix_lapack`, 1431.07742002), con SuperLU (1431.07742001) y con 40 iteraciones de equilibrio (1431.08) — el solve lineal EXACTO no mueve el valor. σn se reparte NO-uniforme por pares de nodos (1609/1609/1073/536/536) y σxx del post_point queda en 1200.00635 con solver directo (target ±1e-3): el punto fijo del esquema escalonado u-σ GNU no equilibra la tracción de penalización de la interfaz con el campo σ del contínuo (misma familia DIAG-SOLVE-MIXTO que mom 0.23×; la σ-dof avanza por update diagonal lumped). PENDIENTE de mecánica de esquema, no de solver.
...
- `interface_bar3_quad8` / `interface_quad8_hex20`: interfaces 3D cuadráticas (Carril B: PARSE `-quad8`/`-hex20` — tipos de elemento no registrados en el GNU).
