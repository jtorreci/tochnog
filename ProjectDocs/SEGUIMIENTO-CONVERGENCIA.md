...
**Sprint 11 — lote 4: familia `control_print_dof_line` (6.273-6.280, 8 keywords) + `control_print_dof_point` (6.281-6.283, 3 keywords)** | `fea2a3c` | 2026-08-27 | interpolación de node_dof Y node_dof_calcul a lo largo de una polilínea / en un punto (nuevo `print_dl.cc`; `point_el()` gana el parámetro opcional `eps_iso` con default 1.e-3 = EPS_ISOP legacy — los 5 callers previos intactos; enums en tochnog.h+tochnog-mod.h en sync; dispatch en top.cc DENTRO del gate `control_print_frequency_allowed` como el resto de prints). **Ficheros**: UNO por dof label (`db_name(dof_label[iuknwn])`, ej. `disy.30` — el "dof" del manual es el LABEL, NO `dof.<index>` de control_print_dof) + uno por item node_dof_calcul (`post_calcul_names_without_extension`, convención `a`+unknown de print_unknowns, ej. `adis.44`); `-separate_index` = label.<icontrol>, `-separate_sequential` = label.<seq> con contador estático por llamada (misma limitación que print_dof). **Decisiones de diseño**: (1) puntos equidistantes sobre la LONGITUD TOTAL de la polilínea (el manual solo dice "how many points"); (2) default de `_n` = 5 (el default legacy de POST_LINE_N; el manual no lo fija); (3) punto fuera de la malla/grupo → OMITIDO de los archivos (coherente con "el punto es aceptado como parte de un elemento"); (4) `_move` solo si `materi_velocity` está inicializado (si no, se ignora; patrón POST_POINT_MOVE con DTIME NEW→NORMAL, coordenadas PUT de vuelta al record); (5) `_method` selecciona el frame del point-in-element Y de las coordenadas escritas, con fallback por nodo a NODE si no existe NODE_START_REFINED; (6) node_dof_calcul SÍ se imprime en line/point siguiendo 6.273/6.281 (el legacy print_dof de 6.269 NO lo imprime — diferencia documentada); (7) `-yes` = `-separate_index`. **VERIFICACIÓN** (tests propios, suite 165/165 = 155 + 10 en build limpio --clean): `dpline1` (interpolación EXACTA: campo lineal disy=-0.01*y reproducido por las funciones de forma — 0/-0.005/-0.01), `dpline_n` (A/B 3 vs 5 líneas; los bloques control_timestep corren EN SERIE: el bloque 32 arranca en t=1), `dpline_geom` (polilínea en V (0,0)->(1,1)->(2,0): los puntos caen sobre los SEGMENTOS (0.5,0.5)/(1,1)/(1.5,0.5), no sobre la recta), `dpline_group` (A/B filtro {1}: punto sobre elemento del group 0 omitido → 2 vs 3 líneas), `dpline_eps` (A/B punto (1.02,0.5) fuera de la malla: default 1.e-3 rechaza, eps=1.0 acepta con extrapolación → 1 vs 2 líneas), `dpline_method` (A/B follow-material: -node_start_refined 2 líneas vs -node 1 — el punto (0.5,0.995) queda 0.005 sobre el borde deformado y=0.99; GOTCHA: con memory -total el matrix_inverse(old_rot) estalla → -updated_without_rotation), `dpline_move` (A/B: y=0.499 (partícula movida) vs 0.5 en la línea 3), `dpline_time`/`dpoint_time` (primera línea `# time 0.1` gnuplot), `dpoint1` (serie temporal 2 pasos -0.001/-0.002 + calcul `adis.44` = media del vector desplazamiento -0.0005/-0.001; GOTCHA del modelo: con fixed_in_space el strain total es 0 (F=I) → el calcul usa el desplazamiento). Checks de archivos en build_safe.sh (conteos + valores exactos + A/B). GOTCHA del script: mawk bajo locale es_ES parsea "0.5" como 0 (strtod con separador ',') → `export LC_ALL=C` al inicio de build_safe.sh. |
